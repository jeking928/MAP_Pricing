...
// check for and ensure that the database contains the required fields for MAP support:
  $fieldOkay1 = (method_exists($sniffer, 'field_type')) ? $sniffer->field_exists(TABLE_PRODUCTS, 'map_enabled', true) : -1;
  if ($fieldOkay1 !== true) {
    $sql = "ALTER TABLE " . TABLE_PRODUCTS . " ADD map_enabled TINYINT NOT NULL DEFAULT '0' AFTER products_priced_by_attribute ,
...
// Map Pricing (edit by www.slickricky.com)
    if (!isset($pInfo->map_enabled)) $pInfo->map_enabled = '0';
    switch ($pInfo->map_enabled) {
      case '0': $is_map_enabled = false; $not_map_enabled = true; break;
      case '1': $is_map_enabled = true; $not_map_enabled = false; break;
      default: $is_map_enabled = false; $not_map_enabled = true;
    }
// Product is Free
...
/* WORK ON THIS GROSS PRICE JAVASCRIPT SLICKRICKY

function updateGross() {
  var taxRate = getTaxRate();
  var grossValue = document.forms["new_product"].products_price.value;
  var grossValue_frommap = document.forms["new_product"].map_price.value;

  if (grossValue_frommap > 0 ) {
	  if (taxRate > 0) {
  		  grossValue_frommap = grossValue_frommap * ((taxRate / 100) + 1);
	  } else if (taxRate > 0) {
  		  grossValue = grossValue * ((taxRate / 100) + 1);
  	}
*/
...
function updateGross_frommap() {
  var taxRate = getTaxRate();
  var grossValue = document.forms["new_product"].map_price.value;
